...
const Clock:FC = ():JSX.Element => {
	...
	useEffect(() => {
		const timer = setInterval(() => setDegreesMinute((prev) => prev + 6), 61000);
		return () => clearInterval(timer);
	}, []);
	...
	return (
		<S.Container >
				<S.Hour
					style={{ transform: `rotate(${degreesHours}deg)` }}
				></S.Hour>
			<S.Minute style={{ transform: `rotate(${degreesMinute}deg)` }}>
			</S.Minute>
			<S.Second style={{ transform: `rotate(${degreesSecond}deg)` }}>
			</S.Second>
		</S.Container>
	);
};
